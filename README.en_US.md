...
`DevOps Service` is the continuous delivery service of Choerodon. Current version: `1.3.3`
...
* `bootstrap.yml`:
  ...
  spring:
    application:
      name: devops-service-business-business
    cloud:
      config:
        failFast: true
        retry:
...
* `application.yml`:
  ...
  agent:
    version: "0.5.0" # expect choerodon-agent version for this devops-service-business-business version
    serviceUrl: "agent.example.com" # url for choerodon-agent to connect devops-service-business-business
...
2. pull source code of `DevOps Service`:
   ...
   ```sh
   git clone https://github.com/choerodon/devops-service-business-business.git
   ```
...
## Reporting Issues
If you find any shortcomings or bugs, please describe them in the  [issue](https://github.com/choerodon/devops-service-business/issues/new).
