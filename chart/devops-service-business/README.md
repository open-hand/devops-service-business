...
```bash
$ helm install c7n/devops-service-business --name devops-service-business
```
...
image.repository| 镜像仓库地址 | registry.cn-shanghai.aliyuncs.com/c7n/devops-service-business
...
env.open.AGENT_SERVICEURL|agent连接devops ws地址|ws://devops-service-business.choerodon.com.cn/agent/
...
service.name|service名字|devops-service-business
service.type|service类型|ClusterIP
service.port|service端口|8060
ingress.enabled|是否创建域名|false
ingress.host|域名地址|devops-service-business.choerodon.com.cn
...
```bash
$ helm install c7n/devops-service-business \
    --set env.open.SKYWALKING_OPTS="-javaagent:/agent/skywalking-agent.jar -Dskywalking.agent.application_code=devops-service-business  -Dskywalking.agent.sample_n_per_3_secs=-1 -Dskywalking.collector.backend_service=oap.skywalking:11800" \
    --name devops-service-business
```

## 验证部署
```bash
curl -s $(kubectl get po -n c7n-system -l choerodon.io/release=devops-service-business -o jsonpath="{.items[0].status.podIP}"):8061/actuator/health | jq -r .status
```
出现以下类似信息即为成功部署
